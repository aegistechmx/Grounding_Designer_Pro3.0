...
// import { generatePotentialCurveChart } from '../chartGenerator'; // Server-side only, uses Node.js libraries
...
export const buildCurveChartSection = async (doc, { calculations, discreteGrid, yPos }) => {
  ...
  try {
    ...
    if (curveData.length > 0) {
      // TODO: Chart generation moved to backend - frontend will use backend-generated charts
      // For now, show placeholder
      doc.setFontSize(9);
      doc.setTextColor(60, 60, 60);
      doc.text('Gráfica de curva generada en el servidor (backend)', 20, yPos + 40);
      yPos += 50;

      /*
      // Generate chart image (server-side only)
