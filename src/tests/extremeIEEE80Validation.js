...
import('../domain/grounding/IEEE80Formulas.js').then(({ default: IEEE80Formulas }) => {
  ...
  console.log('\n=== BASE CASE (Reference) ===');
  const baseResults = runTest('BASE', baseCase);
  ...
  const test1Results = runTest('HIGH RESISTIVITY', highResistivityCase);
  ...
  const test2Results = runTest('EXTREME SURFACE', extremeSurfaceCase);
  ...
  const test3Results = runTest('SMALL GRID', smallGridCase);
  ...
  const test4Results = runTest('TWO-LAYER', twoLayerCase);
...
function runTest(testName, testCase) {
  ...
  try {
    ...
    const Cs = IEEE80Formulas.calculateSurfaceLayerFactor(
      testCase.soil.resistivity || testCase.soil.layer1?.resistivity,
...
function analyzeTwoLayerEffect(testName, base, current) {
  ...
  const effectiveResistivity = (current.testCase.soil.layer1.resistivity + current.testCase.soil.layer2.resistivity) / 2;
