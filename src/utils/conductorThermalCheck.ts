/**
 * Catálogo Viakon - Conductores de cobre (TypeScript)
...
export interface ThermalCheckResult {
  ...
  thermalComplies: boolean;
  ampacityComplies: boolean;
  minRequiredArea: string;
  currentArea: string;
  requiredAmpacity: string;
  currentAmpacity: number;
  faultCurrent: string;
  ...
  recommendedConductor: Conductor;
  parallelInfo: ParallelInfo | null;
...
const THERMAL_CONSTANTS: Record<string, ThermalConstants> = {
 ...
 */
const calculateMinArea = (faultCurrent: number, faultDuration: number, material: string = 'COPPER_SOFT'): number | null => {
 ...
 */
const calculateParallelConductors = (faultCurrent: number, conductorAmpacity: number): number => {
 ...
 */
const findRecommendedConductor = (minArea: number): Conductor => {
 ...
 */
const findCurrentConductor = (currentArea: number): Conductor => {
 ...
 * @param faultCurrent - Corriente de falla (A)
 * @param faultDuration - Duración de la falla (s)
 * @param currentArea - Área actual del conductor (mm²)
 * @param material - Material ('COPPER_SOFT', 'COPPER_HARD', 'ALUMINUM')
 * @returns Resultado completo de verificación
 */
export const conductorThermalCheck = (
  faultCurrent: number,
  faultDuration: number,
  currentArea: number,
  material: string = 'COPPER_SOFT'
): ThermalCheckResult => {
  // Validar material
  const constData = THERMAL_CONSTANTS[material];
  if (!constData) {
    return {
      complies: false,
      severity: 'error',
      thermalComplies: false,
      ampacityComplies: false,
      minRequiredArea: 'N/A',
      currentArea: 'N/A',
      requiredAmpacity: 'N/A',
      currentAmpacity: 0,
      faultCurrent: 'N/A',
      faultDuration: 'N/A',
      material: 'Desconocido',
      formula: 'Cálculo no disponible',
      message: 'Error: Material no soportado',
      recommendation: 'Use COPPER_SOFT, COPPER_HARD o ALUMINUM',
      currentConductor: findCurrentConductor(currentArea),
      recommendedConductor: findRecommendedConductor(0),
      parallelInfo: null,
      needsParallel: false,
      parallelCount: 1,
  ...
  const thermalComplies = currentArea >= (minRequiredArea ?? 0);
  
  // 2. Encontrar conductores
  const currentConductor = findCurrentConductor(currentArea);
  const recommendedConductor = findRecommendedConductor(minRequiredArea ?? 0);
  ...
  let parallelInfo: ParallelInfo | null = null;
  ...
  const severity: 'success' | 'warning' | 'error' = complies ? 'success' : 
    (!thermalComplies && !ampacityComplies) ? 'error' : 
    !thermalComplies ? 'error' : 
    !ampacityComplies ? 'warning' : 'error';
 ...
 */
export const recommendConductor = (
  faultCurrent: number,
  faultDuration: number,
  material: string = 'COPPER_SOFT'
): ConductorRecommendation | null => {
  const minRequiredArea = calculateMinArea(faultCurrent, faultDuration, material);
