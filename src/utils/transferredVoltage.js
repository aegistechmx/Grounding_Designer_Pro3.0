...
export const calculateTransferredVoltage = (GPR, distance, soilResistivity = 100) => {
  ...
  if (!distance || distance <= 0) {
    return {
      GPR: GPR?.toFixed(0) || 'N/A',
      distance: 0,
      theoreticalVoltage: GPR?.toFixed(0) || 'N/A',
      practicalVoltage: GPR?.toFixed(0) || 'N/A',
      risk: GPR > 500 ? 'ALTO' : GPR > 200 ? 'MODERADO' : 'BAJO',
  ...
  return {
    ...
    attenuationConstant: alpha?.toFixed(4) || 'N/A',
    attenuation: attenuation?.toFixed(4) || 'N/A',
    attenuationPercent: (attenuation * 100)?.toFixed(2) || 'N/A',
    theoreticalVoltage: Math.max(0, theoreticalVoltage)?.toFixed(0) || 'N/A',
    practicalVoltage: Math.max(0, practicalVoltage)?.toFixed(0) || 'N/A',
    risk,
    recommendation,
    requiresBonding: theoreticalVoltage > 500
  };
};
...
export const checkEquipotentialBonding = (gpr1, gpr2, distance) => {
  ...
  return {
    voltageDifference: difference?.toFixed(0) || 'N/A',
    isAcceptable,
    severity,
    recommendation: isAcceptable
      ? '✅ Diferencia aceptable, no requiere acción adicional'
      : `⚠️ Diferencia elevada (${difference?.toFixed(0) || 'N/A'} V) - Severidad: ${severity}. Se requiere conductor de enlace equipotencial ${cableSize}`,
    cableSize,
    distance: distance?.toFixed(1) || 'N/A'
  };
};
...
export const calculateSafeDistance = (GPR, soilResistivity = 100, maxVoltage = 500) => {
  ...
  return {
    safeDistance: safeDistance?.toFixed(1) || 'N/A',
    GPR: GPR?.toFixed(0) || 'N/A',
    maxVoltage,
    soilResistivity,
    message: safeDistance > 0 
      ? `Para limitar la tensión transferida a ${maxVoltage} V, mantener distancia mínima de ${safeDistance?.toFixed(1) || 'N/A'} m`
      : `El GPR de ${GPR?.toFixed(0) || 'N/A'} V ya es seguro dentro de la malla`
  };
};
