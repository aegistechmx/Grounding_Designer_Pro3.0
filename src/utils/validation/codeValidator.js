...
export const validateCodes = (params, calculations) => {
  ...
  const safeEtouch70 = Math.max(0.1, calculations.Etouch70 || 1);
  const safeEstep70 = Math.max(0.1, calculations.Estep70 || 1);
  const safetyMarginTouch = calculations.Etouch70 ? (isFinite((safeEtouch70 - (calculations.Em || 0)) / safeEtouch70 * 100) ? ((safeEtouch70 - (calculations.Em || 0)) / safeEtouch70 * 100).toFixed(1) : '0') : '0';
  const safetyMarginStep = calculations.Estep70 ? (isFinite((safeEstep70 - (calculations.Es || 0)) / safeEstep70 * 100) ? ((safeEstep70 - (calculations.Es || 0)) / safeEstep70 * 100).toFixed(1) : '0') : '0';
  const gprRiskLevel = (calculations.GPR || 0) > 10000 ? 'CRITICAL' : (calculations.GPR || 0) > 5000 ? 'HIGH' : (calculations.GPR || 0) > 2000 ? 'MEDIUM' : 'LOW';
    ...
    IEEE80: {
      ...
      requirements: [
        { 
          ...
          value: `${isFinite(calculations.Em) ? calculations.Em.toFixed(0) : 'N/A'} V`,
          limit: `${isFinite(calculations.Etouch70) ? calculations.Etouch70.toFixed(0) : 'N/A'} V`,
          margin: `${safetyMarginTouch}%`
        },
        { 
          name: 'Tensión de paso (Es ≤ Estep 70kg)', 
          passed: calculations.stepSafe70,
          value: `${isFinite(calculations.Es) ? calculations.Es.toFixed(0) : 'N/A'} V`,
          limit: `${isFinite(calculations.Estep70) ? calculations.Estep70.toFixed(0) : 'N/A'} V`,
          margin: `${safetyMarginStep}%`
        },
        { 
          name: 'Resistencia de malla (Rg ≤ 5Ω - recomendado)', 
          passed: calculations.Rg <= 5,
          value: `${isFinite(calculations.Rg) ? calculations.Rg.toFixed(2) : 'N/A'} Ω`,
          limit: '5 Ω',
          margin: calculations.Rg && calculations.Rg !== 0 ? `${isFinite((5 - calculations.Rg) / 5 * 100) ? ((5 - calculations.Rg) / 5 * 100).toFixed(1) : 'N/A'}%` : 'N/A'
        }
      ]
    },
    ...
    NOM001_SEDE: {
      ...
      requirements: [
        ...
        { 
          ...
          value: `${isFinite(calculations.GPR) ? calculations.GPR.toFixed(0) : 'N/A'} V`,
          limit: '5000 V',
...
export const generateCodeRecommendations = (params, calculations) => {
  ...
  if (calculations.Rg > 5) {
    recommendations.push({
      code: 'CFE_01J00-01',
      severity: 'HIGH',
      message: `Resistencia de malla alta (${isFinite(calculations.Rg) ? calculations.Rg.toFixed(2) : 'N/A'} Ω > 5 Ω)`,
  ...
  if (calculations.GPR > 5000) {
    recommendations.push({
      code: 'NOM-001-SEDE',
      severity: 'MEDIUM',
      message: `GPR elevado (${isFinite(calculations.GPR) ? calculations.GPR.toFixed(0) : 'N/A'} V > 5000 V)`,
